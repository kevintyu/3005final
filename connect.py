            ...
            with open(file_path, 'r', encoding='utf-8') as json_file:
                ...
                for event in events:
                    ...
                    if event_type_name in valid_types:
                        ...
                        through_ball = event['pass']['through_ball'] if event_type_name == 'Pass' and 'through_ball' in event['pass'] else False
                        # Prepare SQL INSERT statement
                        insert_query = """
                            INSERT INTO event (eventID, gameID, playerID, XG, FirstTime, IntendedRecipient, Complete, TypeID, throughball)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                            ON CONFLICT (eventID) DO NOTHING;
                        """
                        cursor.execute(insert_query, (eventid, gameid, player_id, xg, first_time, intended_recipient_id, complete, type_id, through_ball))
