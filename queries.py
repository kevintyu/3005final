...
def Q_1(conn, execution_time):
    ...
    query = """SELECT 
                Player.name,
                AVG(event.XG) as average_xG
            FROM 
                Player
            JOIN 
                event ON Player.playerID = event.playerID
            JOIN 
                Game ON event.gameID = Game.GameID
            JOIN 
                Competition ON Game.CompetitionID = Competition.CompetitionID
            JOIN 
                Season ON Game.SeasonID = Season.SeasonID
            WHERE 
                Season.SeasonName = '2020/2021'
                AND Competition.CompetitionName = 'La Liga'
                AND event.type = 'Shot'
                AND event.XG > 0
            GROUP BY 
                Player.name
            HAVING 
                COUNT(event.eventID) > 0
            ORDER BY 
                average_xG DESC;
            """
...
def Q_2(conn, execution_time):
    ...
    query = """ SELECT 
                Player.name,
                COUNT(event.eventID) as shots_count
            FROM 
                Player
            JOIN 
                event ON Player.playerID = event.playerID
...
def Q_3(conn, execution_time):
    ...
    query = """SELECT 
                Player.name,
                COUNT(event.eventID) as first_time_shots_count
            FROM 
                Player
            JOIN 
                event ON Player.playerID = event.playerID
            ...
            JOIN 
                Season ON Game.SeasonID = Season.SeasonID
            WHERE 
                Season.SeasonName IN ('2020/2021', '2019/2020', '2018/2019')
                AND Competition.CompetitionName = 'La Liga'
                AND event.type = 'Shot'
                AND event.FirstTime = TRUE
            GROUP BY 
                Player.name
            HAVING 
                COUNT(event.eventID) >= 1
            ORDER BY 
                first_time_shots_count DESC;
            """
...
def Q_4(conn, execution_time):
    ...
    query = """SELECT 
                T.TeamName,
                COUNT(E.eventID) as pass_count
            FROM 
                Event E
            JOIN 
                Game G ON E.gameID = G.GameID
            JOIN 
                Team T ON T.TeamID = T.TeamID
            JOIN 
                Competition C ON G.CompetitionID = C.CompetitionID
            JOIN 
                Season S ON G.SeasonID = S.SeasonID
            WHERE 
                S.SeasonName = '2020/2021'
                AND C.CompetitionName = 'La Liga'
                AND E.type = 'Pass'
            GROUP BY 
                T.TeamName
            HAVING 
...
def Q_5(conn, execution_time):
    ...
    query = """SELECT 
                ...
                COUNT(E.eventID) AS IntendedPassesCount
            FROM 
                Event E
            JOIN 
                Player P ON E.IntendedRecipient = P.playerID
            JOIN 
                Game G ON E.gameID = G.GameID
            JOIN 
                Competition C ON G.CompetitionID = C.CompetitionID
            JOIN 
                Season S ON G.SeasonID = S.SeasonID
            WHERE 
                S.SeasonName = '2003/2004'
                AND C.CompetitionName = 'Premier League'
                AND E.type = 'Pass'
            GROUP BY 
                P.name
            HAVING 
...
def Q_6(conn, execution_time):
    ...
    query = """SELECT 
            ...
            FROM 
                Event
            JOIN 
                playedIn ON Event.gameID = playedIn.GameID
            JOIN 
                Team ON playedIn.TeamID = Team.TeamID
            JOIN 
                Game ON Event.gameID = Game.GameID
            JOIN 
                Competition ON Game.CompetitionID = Competition.CompetitionID
            JOIN 
                Season ON Game.SeasonID = Season.SeasonID
            WHERE 
                Season.SeasonName = '2003/2004'
                AND Competition.CompetitionName = 'Premier League'
                AND Event.type = 'Shot'
            GROUP BY 
                Team.TeamName
            HAVING 
